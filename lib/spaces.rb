...
class Space
  ...
  def self.create(name:, user_id:, description:, price:, from:, to:)
    ...
    result = connection.exec("INSERT INTO spaces(name, user_id, description, price, from_date, to_date) VALUES('#{name}', '#{user_id}', '#{description}', #{price}, '#{from}', '#{to}') RETURNING id, user_id, name, description, price, from_date, to_date;")
